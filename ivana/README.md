...
**On Windows (MSYS2/MinGW-w64, e.g. the UCRT64 shell):** the Makefile auto-detects
this environment and switches to `-lopengl32 -lgdi32 -lwinmm` instead of the Linux
system libraries, producing `or_defense.exe`. Build raylib the same way inside the
same MSYS2 shell first (`make PLATFORM=PLATFORM_DESKTOP -j4` in `raylib/src`), then
`make` in this project's directory. If you're using a different toolchain (MSVC,
plain `cmd.exe` without MSYS2), build raylib with that toolchain's usual method and
either adapt the Makefile's Windows branch or compile the files in `src/` directly
against your raylib install, linking `opengl32.lib gdi32.lib winmm.lib`.
